...
Keep the extension focused on orchestration. The extension owns phase state, TUI display, model routing, gate coordination, artifact references, and fresh Pi session handoff. The agent phases and skills own the substantive work of writing specs, hardening, task creation, implementation, and review.
...
- Use focused Node tests while iterating, for example `node --test tests/package-validation-contract.test.ts` or `node --test tests/pi-harness-adapter.test.ts`.
- There is no separate build script in `package.json` today; Pi loads the TypeScript source directly.
...
- Keep modules small and responsibility-focused. Prefer adding logic near the existing subsystem:
  ...
  - `src/artifacts/` for artifact paths, references, and session handoff summaries.
...
- If you touch the extension entrypoint, keep `index.ts`, `src/extension-entry.ts`, and local harness imports source-run compatible with explicit `.ts` specifiers.
...
- Session handoff summaries must include enough durable context for the workflow to resume in replacement sessions.
...
- For harness changes, cover both persisted state and user-visible effects such as TUI updates, notifications, session handoff options, and follow-up prompts.
