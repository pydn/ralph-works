      ...
      switch (cmd) {
        case "start": {
          ...
          if (parsedStartArgs.args[0]) {
            ...
            else { promptText = resolvePromptInput(parsedStartArgs.args[0], ctx.cwd); if (parsedStartArgs.args[1]) { const rp = parsedStartArgs.args[1].split(",").map(p => p.trim()).filter(p => validPhases.has(p)); if (rp.length) phases = rp; } }
          }
