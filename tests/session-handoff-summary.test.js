...
} from "../src/artifacts/session-handoff-summary.js";
...
test("artifact inventory safely classifies existing, missing, large, binary, outside, and symlink-escaped artifacts", async (t) => {
  ...
  assert.equal(byKey.get("generatedSpec").status, "present");
  assert.match(byKey.get("generatedSpec").excerpt, /safe handoff artifact/);
  assert.equal(byKey.get("missingThing").status, "missing");
  assert.equal(byKey.get("missingThing").omissionReason, "file does not exist");
  assert.equal(byKey.get("binaryThing").status, "skipped");
  assert.equal(
    byKey.get("binaryThing").omissionReason,
    "binary or non-UTF-8 file",
  );
  assert.equal(byKey.get("outsideThing").status, "skipped");
  assert.equal(byKey.get("outsideThing").omissionReason, "outside workspace");
  assert.equal(byKey.get("unsafeSymlink").status, "skipped");
  assert.equal(
    byKey.get("unsafeSymlink").omissionReason,
    "symlink target escapes workspace",
  );
  assert.equal(byKey.get("largeThing").status, "present");
  assert.ok(byKey.get("largeThing").excerpt.length <= 24);
  assert.equal(
    byKey.get("largeThing").omissionReason,
