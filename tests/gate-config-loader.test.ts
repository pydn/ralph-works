...
import {
  type GateConfig,
  loadGateConfig,
} from "../src/gates/gate-config-loader.ts";
import { validateGateConfig } from "../src/gates/gate-config-validator.ts";

test("missing gate config falls back to an empty TDD gate set", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "ralph-gates-"));
  try {
    const config = (await loadGateConfig(dir)) as GateConfig;
