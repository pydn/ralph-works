...
import type { GateExecutionResult } from "../src/gates/gate-result.ts";
import { registerRalphWorksExtension } from "../src/harness/pi-harness-adapter.ts";
import type {
  NotificationLevel,
  PersistedSessionEntry,
  RalphWorksCommandDefinition,
  RalphWorksContext,
  RalphWorksEventHandler,
  RalphWorksNewSessionOptions,
  RalphWorksNewSessionResult,
  RalphWorksPiApi,
  RalphWorksSessionManager,
  RalphWorksToolDefinition,
  UserMessageOptions,
} from "../src/harness/pi-harness-types.ts";
import { RALPH_WORKS_NEW_SESSION_NOTICE } from "../src/harness/pi-session-handoff.ts";
import { RALPH_WORKS_STATE_ENTRY_TYPE } from "../src/harness/pi-state-persistence.ts";
import { createPhaseState } from "../src/state/phase-state.ts";
import { transitionToPhase } from "../src/state/phase-transitions.ts";
import type { PhaseId, WorkflowState } from "../src/state/phase-types.ts";
import {
...
type RequiredMap<K, V> = Omit<Map<K, V>, "get" | "set"> & {
  get(key: K): V;
  set(key: K, value: V): RequiredMap<K, V>;
};
...
interface RegisteredToolExecutionResult {
  content: { type: "text"; text: string }[];
  details: { state: WorkflowStateWithHandoff };
}

interface RegisteredToolDefinition
  extends Omit<RalphWorksToolDefinition, "execute"> {
  execute: (
    ...args: Parameters<RalphWorksToolDefinition["execute"]>
  ) => RegisteredToolExecutionResult | Promise<RegisteredToolExecutionResult>;
}

type RequiredToolList = RecordedArray<RegisteredToolDefinition> & {
  find(
    predicate: (
      value: RegisteredToolDefinition,
      index: number,
      obj: RegisteredToolDefinition[],
    ) => unknown,
...
interface SentContextMessage {
  content: string;
  options: UserMessageOptions;
}

interface FakePiCalls {
  commands: RequiredMap<string, RalphWorksCommandDefinition>;
  events: RequiredMap<string, RalphWorksEventHandler>;
  tools: RequiredToolList;
  appended: RecordedArray<{ customType: string; data: WorkflowState }>;
  models: RecordedArray<ModelCall>;
  userMessages: RecordedArray<SentContextMessage>;
  operations: RecordedArray<
    | { type: "setModel"; model: unknown }
    | { type: "sendUserMessage"; content: string; options?: UserMessageOptions }
    | {
...
interface FakeContextCalls {
  statuses: RecordedArray<{ key: string; value: string | undefined }>;
  widgets: RecordedArray<{ key: string; value: string[] }>;
  notifications: RecordedArray<{ message: string; level?: NotificationLevel }>;
  newSessions: RecordedArray<RalphWorksNewSessionOptions>;
  replacementContexts: RecordedArray<TestContext>;
}

interface TestSessionManager extends RalphWorksSessionManager {
...
type TestContext = Omit<
  RalphWorksContext,
  "sessionManager" | "newSession" | "sendUserMessage"
> & {
...
interface LifecycleHarness {
  ...
  readonly activeRuntime: LifecycleRuntime;
  readonly runtimes: LifecycleRuntime[];
}
...
function createFakePi({
  ...
  exec?: FakeExec;
} = {}): { calls: FakePiCalls; pi: RalphWorksPiApi } {
  const calls: FakePiCalls = {
    commands: new Map() as RequiredMap<string, RalphWorksCommandDefinition>,
    events: new Map() as RequiredMap<string, RalphWorksEventHandler>,
    tools: recordedArray<RegisteredToolDefinition>() as RequiredToolList,
    appended: recordedArray<{ customType: string; data: WorkflowState }>(),
    models: recordedArray<ModelCall>(),
    userMessages: recordedArray<SentContextMessage>(),
    operations: recordedArray<FakePiCalls["operations"][number]>(),
  };
  ...
  return {
    ...
    pi: {
      ...
      registerTool(definition) {
        calls.tools.push(definition as RegisteredToolDefinition);
      },
      appendEntry(customType, data) {
        calls.appended.push({ customType, data });
      },
      async setModel(model) {
        calls.models.push(model as ModelCall);
      ...
      sendUserMessage(content, options) {
        ...
        calls.userMessages.push({ content, options: messageOptions });
...
function createFakeContext(
...
): { calls: FakeContextCalls; ctx: TestContext } {
  const calls: FakeContextCalls = {
    statuses: recordedArray<{ key: string; value: string | undefined }>(),
    widgets: recordedArray<{ key: string; value: string[] }>(),
    notifications: recordedArray<{
      message: string;
  ...
  ctx.newSession = async (options: RalphWorksNewSessionOptions) => {
    ...
    const replacementEntries: PersistedSessionEntry[] = [];
    const replacementSessionManager: TestSessionManager = {
    ...
    const replacementCtx: TestContext = {
      ...ctx,
...
type WorkflowStateWithHandoff = Omit<
  WorkflowState,
  | "completedPhases"
  | "gateResults"
  ...
  | "transitionHistory"
> & {
  completedPhases: RecordedArray<WorkflowState["completedPhases"][number]>;
  gateResults: RecordedArray<WorkflowState["gateResults"][number]>;
  implementationStatus: NonNullable<WorkflowState["implementationStatus"]> & {
    claimedTaskIds: RecordedArray<string>;
    completedTaskIds: RecordedArray<string>;
  };
  ...
  sessionHandoffEvents: RecordedArray<
    WorkflowState["sessionHandoffEvents"][number]
  >;
  transitionHistory: RecordedArray<WorkflowState["transitionHistory"][number]>;
};

function latestState(piCalls: FakePiCalls): WorkflowStateWithHandoff {
  const state = piCalls.appended.at(-1)?.data;
  assert.ok(state, "expected a persisted RalphWorks state");
  return state as WorkflowStateWithHandoff;
}
...
async function completeLatestHandoff(
  _ctxCalls: FakeContextCalls,
  piCalls: FakePiCalls,
  ctx: TestContext,
): Promise<void> {
...
async function advanceToHardenSpecWithNext(
  piCalls: FakePiCalls,
  ctx: TestContext,
): Promise<void> {
...
async function requestAndApproveHardenSpec(
  piCalls: FakePiCalls,
  ctx: TestContext,
): Promise<void> {
  await piCalls.commands.get("ralph-works").handler("next", ctx);
...
async function advanceToTddWithApproval(
  piCalls: FakePiCalls,
  ctx: TestContext,
): Promise<void> {
...
async function advanceToReviewWithApproval(
  piCalls: FakePiCalls,
  ctx: TestContext,
): Promise<void> {
...
function createHandoffContexts(
  tempDir: string,
  sourceState: WorkflowState,
): {
  replacementCtx: TestContext;
  replacementCalls: FakeContextCalls;
  replacementEntries: PersistedSessionEntry[];
  sourceCtx: TestContext;
  sourceCalls: FakeContextCalls;
} {
  const sourceEntries: PersistedSessionEntry[] = [
    {
      type: "custom",
  ...
  const { ctx: sourceCtx, calls: sourceCalls } = createFakeContext(tempDir, {
    ...
    newSession: async (options: RalphWorksNewSessionOptions) => {
      await options.setup(replacementSessionManager);
      await options.withSession(replacementCtx);
      return { cancelled: false };
    },
...
async function runQueuedHandoffThroughReplacement(
  ...
  const resumeMessage = piCalls.userMessages.at(-1);
  assert.ok(resumeMessage, "expected queued resume handoff command");
  assert.equal(
...
async function writeFeatureTaskList(
  tempDir: string,
  markdown: string,
): Promise<void> {
...
function createLifecycleHarness(
  ...
  const runtimes: LifecycleRuntime[] = [];
  let activeRuntime: LifecycleRuntime | undefined;
  ...
  return {
    firstRuntime,
    get activeRuntime() {
      assert.ok(activeRuntime, "expected active lifecycle runtime");
...
async function runActiveLifecycleHandoff(
  ...
  const resumeMessage = replacementRuntime.piCalls.userMessages.at(-1);
  assert.ok(resumeMessage, "expected queued resume handoff command");
  assert.equal(
    resumeMessage.content,
    `/ralph-works resume-handoff ${handoffId}`,
...
test("extension registers ralph-works command, tools, and skill discovery", async () => {
  ...
  const { ctx } = createFakeContext(process.cwd());
...
test("internal handoff command creates a replacement session without old-session continuation", async () => {
  ...
  try {
    ...
    const replacementEntries: PersistedSessionEntry[] = [];
    const replacementSessionManager: RalphWorksSessionManager = {
      appendCustomEntry(customType: string, data: WorkflowState) {
        replacementEntries.push({ type: "custom", customType, data });
      },
      appendCustomMessageEntry(
    ...
    assert.equal(
      latestState(piCalls).pendingHandoff.status,
      "in_progress",
    ...
    assert.equal(
      (replacementEntries[0].data as WorkflowStateWithHandoff).pendingHandoff
        .status,
...
test("failed handoffs block advancement and internal execution until reset", async () => {
  ...
  try {
    ...
    assert.equal(piCalls.appended.length, appendedEntries);
    assert.equal(
      piCalls.appended.length > 0
        ? piCalls.appended.at(-1).data.currentPhase
        : undefined,
      undefined,
    );
...
test("ralph_works_transition creates a replacement session without queueing a slash command", async () => {
  ...
  try {
    ...
    const contextMessages = recordedArray<SentContextMessage>();
    ctx.sendUserMessage = async (content, options) => {
      contextMessages.push({ content, options: options ?? {} });
...
test("replacement handoff routes model before sending the replacement prompt without old runtime sends", async () => {
  ...
  try {
    ...
    replacementCtx.sessionManager = replacementSessionManager;
    const replacementContextMessages = recordedArray<SentContextMessage>();
    replacementCtx.sendUserMessage = async (content, options) => {
      replacementContextMessages.push({ content, options: options ?? {} });
...
test("replacement phase completion reuses replacement session control when event ctx lacks newSession", async () => {
  const tempDir = await mkdtemp(path.join(tmpdir(), "ralph-adapter-"));
  try {
    const { pi, calls: piCalls } = createFakePi();
    const replacementEventContexts = recordedArray<TestContext>();
    let replacementSessionNumber = 0;
    let sourceCtx: TestContext | undefined;
    const { ctx, calls: sourceCalls } = createFakeContext(tempDir, {
      newSession: async (options: RalphWorksNewSessionOptions) => {
        assert.ok(sourceCtx, "expected source context before handoff");
        replacementSessionNumber += 1;
        const sessionFile = `sessions/replacement-${replacementSessionNumber}.jsonl`;
        const replacementEntries: PersistedSessionEntry[] = [];
        const replacementSessionManager: TestSessionManager = {
          appendCustomEntry(customType: string, data: WorkflowState) {
            replacementEntries.push({ type: "custom", customType, data });
        ...
        const { ctx: replacementEventCtx } = createFakeContext(tempDir, {
          entries: replacementEntries,
          parentSession: sessionFile,
        });
        replacementEventCtx.sessionManager = replacementSessionManager;
        delete replacementEventCtx.newSession;
        const replacementCommandCtx: TestContext = {
          ...replacementEventCtx,
          newSession: sourceCtx.newSession,
        };
        replacementEventContexts.push(replacementEventCtx);

        await options.setup(replacementSessionManager);
        await options.withSession(replacementCommandCtx);
        return { cancelled: false };
      },
    });
    sourceCtx = ctx;
    ...
    await startPipeline(piCalls, sourceCtx);
    await finishAssistantTurn(
      piCalls,
      sourceCtx,
      "Spec complete.\nRALPH_PHASE_COMPLETE",
    ...
    const redTeamCtx = replacementEventContexts[0];
    assert.ok(redTeamCtx, "expected red-team replacement event context");
    assert.equal(redTeamCtx.newSession, undefined);
    assert.equal(latestState(piCalls).currentPhase, "red_team");
    assert.equal(latestState(piCalls).phaseStatus, "executing");
    ...
    assert.equal(sourceCalls.newSessions.length, 2);
    assert.equal(
      sourceCalls.newSessions[1].parentSession,
      "sessions/replacement-1.jsonl",
    );
    assert.equal(latestState(piCalls).currentPhase, "harden_spec");
    assert.equal(latestState(piCalls).phaseStatus, "executing");
    assert.equal(latestState(piCalls).pendingHandoff, undefined);
