...
import { requiredGatesPassed } from "../src/gates/gate-result.ts";
import { runConfiguredGates } from "../src/gates/gate-runner.ts";
...
test("gate runner executes gates serially through the supplied executor", async () => {
  ...
  const results = await runConfiguredGates(config, {
    executor: async (gate) => {
